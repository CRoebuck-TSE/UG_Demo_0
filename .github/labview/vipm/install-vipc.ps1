...
$PublicRepoUrl    = if ($Env:VIPM_PUBLIC_REPO_URL) { $Env:VIPM_PUBLIC_REPO_URL } else { 'https://github.com/CRoebuck-TSE/UG_Demo_0.git' }
...
function Save-PublicVipmPackage($Package, [string] $OutDir) {
    ...
    Invoke-WebRequest -Uri $url -OutFile $outFile -UseBasicParsing -MaximumRedirection 10 -TimeoutSec 300 -Headers @{ 'User-Agent' = 'UG_Demo_0 VIPM downloader' } | Out-Null
